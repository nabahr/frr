				    ...
				    struct interface **ifpp)
{
	...
	if (!pim_nht_lookup(pim, &nexthop, mtracep->src_addr, PIMADDR_ANY, 1)) {
		if (PIM_DEBUG_MTRACE)
				    ...
				    struct interface *interface)
{
	...
	if (interface == NULL) {
		memset(&nexthop, 0, sizeof(nexthop));
		/* TODO Is there any valid group address to use for lookup? */
		if (!pim_nht_lookup(pim, &nexthop, ip_hdr->ip_dst, PIMADDR_ANY, 0)) {
			if (PIM_DEBUG_MTRACE)
				zlog_debug(
				...
				struct igmp_mtrace *mtracep, size_t mtrace_len)
{
	...
	if (IPV4_CLASS_DE(ntohl(mtracep->rsp_addr.s_addr))) {
		 ...
		 */
		if (!pim_nht_lookup(pim, &nexthop, mtracep->rsp_addr, PIMADDR_ANY, 1)) {
			if (PIM_DEBUG_MTRACE)
				zlog_debug(
