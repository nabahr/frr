...
def test_pim_mrib_rpf_lookup_group_list(request):
    ...
    with McastTesterHelper(tgen) as apphelper:
        step(("Send multicast traffic from R1 to dense groups {}, {}").format(GROUP1, GROUP2))
        ...
        step("Switch group1 to lower distance match (MRIB), and group2 to longer prefix (URIB)")
        tgen.routers()["r4"].vtysh_cmd(
            '''
            conf term
             router pim
        ...
        result = verify_upstream_iif(tgen, "r4", "Unknown", "10.0.1.1", GROUP1, "NotJoined")
        assert result is True, "Testcase {} :Failed \n Error: {}".format(tc_name, result)
