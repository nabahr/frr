...
TOPOLOGY = """
All ip prefix 10.0.x.x

        20.1     20.2  102.2   102.100
      rp ----------- r2 ------------- h2
  30.1 \
        --------- r3 ------------- h3
              30.3  103.3   103.100

RP is on the loopback 10.254.0.1/32
All static routes
"""
...
def build_topo(tgen):
    ...
    # tgen.add_router("r3")
    tgen.add_router("rp")
    tgen.add_host("h2", "10.0.102.100/24", "via 10.0.102.1")
    tgen.add_host("h3", "10.0.103.100/24", "via 10.0.103.1")

    sw = tgen.add_switch("sw1")
    sw.add_link(tgen.gears["rp"])
    sw.add_link(tgen.gears["h2"])

    sw = tgen.add_switch("sw2")
    sw.add_link(tgen.gears["rp"])
    sw.add_link(tgen.gears["h3"])
...
def setup_module(mod):
    logger.info("PIM basic functionality:\n {}".format(TOPOLOGY))
